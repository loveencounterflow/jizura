(function() {
  var $, $async, CHR, CND, D, HOLLERITH, after, alert, badge, debug, echo, eventually, every, help, illegal_component, immediately, info, join, log, njs_fs, njs_path, options, repeat_immediately, rpr, step, suspend, urge, warn, whisper, ƒ,
  ...
  this.read_global_sims = function(db, handler) {
    var Z, count, input, query, ref, source_glyph, target_glyph;
    count = 0;
    Z = {};
    ref = options['cjvki-jzr-sims'];
    for (source_glyph in ref) {
      target_glyph = ref[source_glyph];
      count += +1;
    ...
    return input.pipe($((function(_this) {
      return function(phrase, send) {
        var _;
        _ = phrase[0], _ = phrase[1], target_glyph = phrase[2], source_glyph = phrase[3];
        Z[source_glyph] = target_glyph;
        count += +1;
        return send(source_glyph);
      };
    })(this))).pipe(this.$show_progress(1e4)).pipe(D.$on_end((function(_this) {
      return function() {
  ...
  this.$filter_outer_mapped_and_unknown_glyphs_A = function(glyph_categories) {
  ...
  this.$filter_outer_mapped_and_unknown_glyphs_B = function(global_sims) {
    var counts, unknown_non_cjk_xe;
    counts = {
      'cp/inner/original': 0,
      'cp/inner/mapped': 0,
    ...
    return $((function(_this) {
      return function(fields, send, end) {
        var category, cjkvi_formulas, csg, source_glyph, target_glyph;
        if (fields != null) {
          source_glyph = fields[0], cjkvi_formulas = fields[1];
          csg = CHR.as_csg(source_glyph, {
            input: 'xncr'
          });
          target_glyph = global_sims[source_glyph];
          if (csg === 'u' || csg === 'jzr') {
            if (target_glyph != null) {
              category = 'cp/inner/mapped';
            } else {
              category = 'cp/inner/original';
            }
          } else {
            if (target_glyph != null) {
              category = 'cp/outer/mapped';
            } else {
              category = 'cp/outer/original';
            }
          }
          counts[category] += +1;
          if (category === 'cp/inner/original') {
            send([source_glyph, cjkvi_formulas]);
          }
        }
        if (end != null) {
          help("filtering counts:");
  ...
  this.normalize_formula = (function(_this) {
    return function(global_sims, formula) {
      var chr, chrs, i, idx, len, new_chr, ref;
      chrs = CHR.chrs_from_text(formula, {
        input: 'xncr'
      });
      for (idx = i = 0, len = chrs.length; i < len; idx = ++i) {
        chr = chrs[idx];
        chrs[idx] = new_chr = (ref = global_sims[chr]) != null ? ref : chr;
        if (new_chr === illegal_component) {
          throw new Error("illegal component in formula " + (rpr(formula)));
        }
      }
      return chrs.join('');
    };
  ...
  this.$normalize_formulas = function(global_sims) {
    var counts;
    counts = {
      'jzr': 0,
      'cjkvi': 0
    ...
    return $((function(_this) {
      return function(fields, send, end) {
        var cjkvi_formulas, formula, glyph, i, idx, jzr_formulas, len, normalized_formula;
        if (fields != null) {
          glyph = fields[0], cjkvi_formulas = fields[1], jzr_formulas = fields[2];
          for (idx = i = 0, len = cjkvi_formulas.length; i < len; idx = ++i) {
            formula = cjkvi_formulas[idx];
            normalized_formula = _this.normalize_formula(global_sims, formula);
            if (normalized_formula !== formula) {
              counts['cjkvi'] += +1;
  ...
  this.compare = function() {
    ...
    return step((function(_this) {
      return function*(resume) {
        var global_sims;
        global_sims = (yield _this.read_global_sims(db, resume));
        return input.pipe(D.$split()).pipe(_this.$filter_comments_and_empty_lines()).pipe(D.$parse_csv({
          headers: false,
          delimiter: '\t'
        })).pipe(_this.$resolve_cjkvi_kernel()).pipe(_this.$normalize_cjkvi_ncrs()).pipe(_this.$check_glyph_reference()).pipe(_this.$show_progress(1e4)).pipe(_this.$filter_outer_mapped_and_unknown_glyphs_B(global_sims)).pipe(_this.$remove_region_annotations()).pipe(_this.$retrieve_jzr_formulas(db)).pipe(_this.$normalize_formulas(global_sims)).pipe(_this.$compare_formulas());
      };
    })(this));
  };
