(function() {
  var CND, alert, app, app_name, badge, debug, echo, get_factor_sample, get_glyph_sample, get_stats_route, help, info, log, ref, rpr, urge, warn, whisper;
  ...
  get_stats_route = function(input, fallback) {
    if (fallback == null) {
      fallback = null;
    }
    if (input == null) {
  ...
  app.command("kwic <output_route>").description("render (excerpt of) KWIC index to <output_route>").option("-s --stats [stats-filename]", "render KWIC infix statistics").option("-g --glyphs [glyphs]", "which glyphs to include").option("-f --factors [factors]", "which factors to include").action(function(output_route, options) {
    var S, SHOW_KWIC_V3, factor_sample, glyph_sample, stats_route;
    ...
    if (stats_route != null) {
      ...
      help("no statistics will be written");
    }
