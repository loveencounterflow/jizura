(function() {
  var CND, alert, app, app_name, badge, debug, echo, get_boolean, get_colors, get_do_stats, get_factor_sample, get_glyph_sample, get_two_stats, get_uchrs, get_width, get_with_prefixes, help, info, is_tty, isa_folder, log, njs_fs, njs_path, ref, rpr, urge, warn, whisper;
  ...
  is_tty = get_do_stats = get_with_prefixes = get_uchrs = get_two_stats = get_boolean = function(input, fallback) {
    if (fallback == null) {
      fallback = false;
    }
    if (input == null) {
  ...
  get_colors = function(input, is_tty, fallback) {
    if (fallback == null) {
      fallback = false;
    }
    if (input != null) {
  ...
  app.command("formulas").description("dump formulas for glyphs").option("-g, --glyphs [glyphs]", "glyphs to be searched for").option("-c, --colors", "use colors even when redirecting to file [false]").option("-u, --uchrs", "use characters instead of NCRs for PUA codepoints [false]").option("-i, --noidcs", "omit IDCs [false]").option("-s, --sort", "sort results by CID [false]").action(function(options) {
    var DFFG, S, XNCHR, colors, glyphs, noidcs, ref, sort, uchrs;
    XNCHR = require('./XNCHR');
    DFFG = require('./dump-formulas-for-glyphs');
    glyphs = XNCHR.chrs_from_text((ref = options['glyphs']) != null ? ref : '');
