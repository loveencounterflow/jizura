(function() {
  ...
  this.pdf_from_md = function(source_route, handler) {
    return step((function(_this) {
      return function*(resume) {
        ...
        input.pipe(TYPO.$fix_typography_for_tex()).pipe(_this.MKTX.DOCUMENT.$open(state)).pipe(_this.MKTX.COMMAND.$new_page(state)).pipe(_this.MKTX.REGION.$correct_p_tags(state)).pipe(_this.MKTX.REGION.$filter_empty_p_tags(state)).pipe(_this.MKTX.REGION.$keep_lines(state)).pipe(_this.MKTX.BLOCK.$heading(state)).pipe(_this.MKTX.BLOCK.$paragraph(state)).pipe(_this.MKTX.BLOCK.$hr(state)).pipe(_this.MKTX.INLINE.$code(state)).pipe(_this.MKTX.INLINE.$em_and_strong(state)).pipe(_this.MKTX.DOCUMENT.$close(state)).pipe(TYPO.$show_mktsmd_events(state)).pipe(_this.$filter_tex()).pipe(tex_output);
        return input.resume();
      };
    })(this));
  };
  ...
  this.MKTX.DOCUMENT.$open = (function(_this) {
    return function(S) {
      return $(function(event, send) {
  ...
  this.MKTX.DOCUMENT.$close = (function(_this) {
    return function(S) {
      return $(function(event, send) {
        if (TYPO.isa(event, '}', 'document')) {
          send(_this.stamp(event));
  ...
  this.MKTX.REGION.$correct_p_tags = (function(_this) {
    return function(S) {
      var within_p;
      within_p = false;
      return $(function(event, send) {
  ...
  this.MKTX.REGION.$filter_empty_p_tags = (function(_this) {
    return function(S) {
      var _send, last_event, last_was_open_p, send_later;
      last_was_open_p = false;
      last_event = null;
      _send = null;
      send_later = function(event) {
        if (last_event != null) {
          _send(last_event);
        }
    ...
    return function(S) {
      return $(function(event, send) {
        ...
        if (TYPO.isa(event, ['[', ']'], 'p')) {
          ...
          if (!(S.within_pre || S.within_keeplines || S.just_closed_keeplines)) {
            if (type === '[') {
              send(_this.stamp(event));
              send(['text', '\n\n']);
            } else {
