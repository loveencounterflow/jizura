(function() {
        ...
        if (event != null) {
          if (!TYPO.isa(event, ['{', '}'], 'single-column')) {
            ...
            if (type === '{') {
              ...
              S.within_single_column = true;
              if (S.within_multicol) {
                send(['tex', '\\end{multicols}']);
                S.within_multicol = false;
              }
              send(['tex', '\n\n']);
            } else {
              debug('©x1ESw', ')single-column---------------------------');
              send(['tex', '\\begin{multicols}{2}\n']);
              S.within_multicol = true;
              S.within_single_column = false;
            }
          }
        }
        if (end != null) {
  ...
  this.MKTX.REGION.$keep_lines = (function(_this) {
    return function(S) {
      return $(function(event, send) {
        var meta, name, text, type, within_keeplines, within_pre;
