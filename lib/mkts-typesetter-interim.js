(function() {
  ...
  this.pdf_from_md = function(source_route, handler) {
    ...
    input.pipe(TYPO.$fix_typography_for_tex()).pipe(TYPO.$show_mktsmd_events()).pipe(this.MKTX.$command_new_page()).pipe(this.MKTX.$region_single_column()).pipe(this.$filter_tex()).pipe(D.$show()).pipe(this.$insert_preamble(layout_info)).pipe(this.$insert_postscript()).pipe(tex_output);
    return input.resume();
  };

  this.MKTX = {};

  this.MKTX.$command_new_page = function() {
  ...
  this.MKTX.$region_single_column = function() {
    return $((function(_this) {
      return function(event, send) {
