  ...
  this.write_mkts_master = function(layout_info, handler) {
    return step((function(_this) {
      return function*(resume) {
        var content_locator, defs, filename, fonts_home, fontspec_version, home, i, len, lines, main_font_name, master_locator, name, newcommands, ref1, ref2, texname, text, use_new_syntax, value, write;
        lines = [];
      ...
      return $(function(event, send) {
        ...
        if (TYPO.isa(event, ['[', ']'], ['h1', 'h2', 'h3', 'h4', 'h5', 'h6'])) {
          ...
          if (type === '[') {
            ...
            switch (name) {
              case 'h1':
                return send(['tex', "\\mktsChapter{"]);
              case 'h2':
                return send(['tex', "\\mktsSection{"]);
              default:
                return send(['tex', "\\mktsSubsection{"]);
            }
          } else {
    ...
    return function(S) {
      return $(function(event, send) {
        ...
        if (TYPO.isa(event, ['(', ')'], 'code')) {
          ...
          if (type === '(') {
            return send(['tex', "{\\mktsFontSourcecodeproregular{}"]);
          } else {
            return send(['tex', "}"]);
          }
        } else {
