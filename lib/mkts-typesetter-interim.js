    ...
    return function(S) {
      ...
      return $(function(event, send) {
        ...
        if (TYPO.isa(event, ['{', '}'], 'single-column')) {
          ...
          type = event[0], name = event[1], text = event[2], meta = event[3];
          if (type === '{') {
            if ((!S.within_single_column) && S.within_multi_column) {
              send(_this.MKTX.REGION._end_multi_column());
    ...
    return function(S) {
      ...
      return $(function(event, send) {
        ...
        if (TYPO.isa(event, ['[', ']'], ['h1', 'h2', 'h3', 'h4', 'h5', 'h6'])) {
          ...
          type = event[0], name = event[1], text = event[2], meta = event[3];
          if (type === '[') {
            if (S.within_multi_column && (name === 'h1' || name === 'h2')) {
