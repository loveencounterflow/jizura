    ...
    return D.run((function(_this) {
      return function() {
        return step(function*(resume) {
          ...
          input.pipe(MKTS.$fix_typography_for_tex(_this.options)).pipe(_this.MKTX.DOCUMENT.$begin(state)).pipe(_this.MKTX.DOCUMENT.$end(state)).pipe(_this.MKTX.MIXED.$raw(state)).pipe(_this.MKTX.COMMAND.$do(state)).pipe(_this.MKTX.COMMAND.$expansion(state)).pipe(_this.MKTX.COMMAND.$new_page(state)).pipe(_this.MKTX.COMMAND.$comment(state)).pipe(_this.MKTX.COMMAND.$multi_column(state)).pipe(_this.MKTX.REGION.$multi_column(state)).pipe(_this.MKTX.REGION.$single_column(state)).pipe(_this.MKTX.REGION.$keep_lines(state)).pipe(_this.MKTX.REGION.$code(state)).pipe(_this.MKTX.BLOCK.$heading(state)).pipe(_this.MKTX.BLOCK.$hr(state)).pipe(_this.MKTX.BLOCK.$unordered_list(state)).pipe(_this.MKTX.INLINE.$code(state)).pipe(_this.MKTX.INLINE.$translate_i_and_b(state)).pipe(_this.MKTX.INLINE.$em_and_strong(state)).pipe(_this.MKTX.BLOCK.$paragraph(state)).pipe(_this.MKTX.CLEANUP.$remove_empty_texts(state)).pipe(MKTS.$close_dangling_open_tags(state)).pipe(MKTS.$show_mktsmd_events(state)).pipe(MKTS.$write_mktscript(state)).pipe(MKTS.$show_unhandled_tags(state)).pipe(_this.$filter_tex()).pipe(MKTS.$show_illegal_chrs(state)).pipe(tex_output);
          return input.resume();
        });
      };
    })(this), (function(_this) {
