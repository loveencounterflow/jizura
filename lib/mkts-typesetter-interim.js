(function() {
  ...
  this.pdf_from_md = function(source_route, handler) {
    ...
    input.pipe(TYPO.$fix_typography_for_tex()).pipe(TYPO.$show_mktsmd_events()).pipe(this.$translate_new_page()).pipe(this.$filter_tex()).pipe(this.$insert_preamble(layout_info)).pipe(this.$insert_postscript()).pipe(tex_output);
    return input.resume();
  };

  this.$translate_new_page = function() {
