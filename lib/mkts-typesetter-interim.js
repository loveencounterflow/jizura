  ...
  this.pdf_from_md = function(source_route, handler) {
    return step((function(_this) {
      return function*(resume) {
        ...
        input.pipe(TYPO.$fix_typography_for_tex()).pipe(_this.MKTX.DOCUMENT.$begin(state)).pipe(_this.MKTX.COMMAND.$new_page(state)).pipe(_this.MKTX.REGION.$correct_p_tags(state)).pipe(_this.MKTX.REGION.$keep_lines(state)).pipe(_this.MKTX.BLOCK.$heading(state)).pipe(_this.MKTX.BLOCK.$paragraph(state)).pipe(_this.MKTX.BLOCK.$hr(state)).pipe(_this.MKTX.INLINE.$code(state)).pipe(_this.MKTX.INLINE.$translate_i_and_b(state)).pipe(_this.MKTX.INLINE.$em_and_strong(state)).pipe(_this.MKTX.DOCUMENT.$end(state)).pipe(TYPO.$show_mktsmd_events(state)).pipe(_this.$show_unhandled_tags(state)).pipe(_this.$filter_tex()).pipe(tex_output);
        return input.resume();
      };
    })(this));
  };
