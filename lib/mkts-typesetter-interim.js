(function() {
  var $, $async, ASYNC, CACHE, CND, D, HELPERS, MKTS, OPTIONS, SEMVER, TEXLIVEPACKAGEINFO, alert, badge, copy, debug, echo, help, hide, info, log, njs_fs, njs_path, options_route, ref, rpr, select, stamp, step, suspend, urge, warn, whisper, ƒ,
  ...
  this.MKTX.COMMAND.$definition = (function(_this) {
    return function(S) {
      ...
      var identifier, track, values;
      track = MKTS.TRACKER.new_tracker('(:)', '[:]');
      identifier = null;
      values = {};
      return $(function(event, send) {
        var _, definition, i, len, meta, results, sub_event, type, within_definition;
        ...
        if (select(event, ['(', '['], ':')) {
          ...
          (values[identifier] != null ? values[identifier] : values[identifier] = []).push(event);
    ...
    return function(S) {
      ...
      return $(function(event, send) {
        ...
        if (within_latex && select(event, '.', 'text')) {
          ...
          return send(stamp(['.', 'latex', raw_text, meta]));
  ...
  this.$show_unhandled_tags = function(S) {
    return $((function(_this) {
      return function(event, send) {
        ...
        var event_tex, event_txt, first, last, meta, name, post, pre, ref1, ref2, ref3, text, type;
        if ((ref1 = event[0]) === 'tex' || ref1 === 'text') {
          return send(event);
        } else if (select(event, '.', 'text')) {
          return send(event);
        } else if (!MKTS.is_stamped(event)) {
          type = event[0], name = event[1], text = event[2], meta = event[3];
          if (text != null) {
            if (CND.isa_pod(text)) {
          ...
          if (indexOf.call(MKTS.FENCES.xleft, type) >= 0) {
            ref2 = [type, name], first = ref2[0], last = ref2[1];
            pre = '█';
            post = '';
          } else {
            ref3 = [name, type], first = ref3[0], last = ref3[1];
            pre = '';
            post = '█';
          }
  ...
  this.pdf_from_md = function(source_route, handler) {
    return step((function(_this) {
      return function*(resume) {
        ...
        input.pipe(MKTS.$fix_typography_for_tex(_this.options)).pipe(_this.MKTX.DOCUMENT.$begin(state)).pipe(_this.MKTX.COMMAND.$definition(state)).pipe(_this.MKTX.COMMAND.$new_page(state)).pipe(_this.MKTX.REGION.$correct_p_tags_before_regions(state)).pipe(_this.MKTX.REGION.$multi_column(state)).pipe(_this.MKTX.REGION.$single_column(state)).pipe(_this.MKTX.REGION.$keep_lines(state)).pipe(_this.MKTX.REGION.$code(state)).pipe(_this.MKTX.BLOCK.$remove_empty_p_tags(state)).pipe(_this.MKTX.BLOCK.$heading(state)).pipe(_this.MKTX.BLOCK.$paragraph(state)).pipe(_this.MKTX.BLOCK.$hr(state)).pipe(_this.MKTX.INLINE.$code(state)).pipe(_this.MKTX.INLINE.$latex(state)).pipe(_this.MKTX.INLINE.$translate_i_and_b(state)).pipe(_this.MKTX.INLINE.$em_and_strong(state)).pipe(_this.MKTX.DOCUMENT.$end(state)).pipe(MKTS.$show_mktsmd_events(state)).pipe(MKTS.$write_mktscript(state)).pipe(_this.$show_unhandled_tags(state)).pipe(_this.$filter_tex()).pipe(tex_output);
        return input.resume();
      };
    })(this));
  };
