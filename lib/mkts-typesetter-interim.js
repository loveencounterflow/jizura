(function() {
  ...
  this.MKTX.MIXED.$raw = (function(_this) {
    return function(S) {
      ...
      return $(function(event, send) {
        ...
        track(event);
        if (within_raw && select(event, '.', 'text')) {
          type = event[0], name = event[1], text = event[2], meta = event[3];
          raw_text = meta['raw'];
  ...
  this.MKTX.CLEANUP.$remove_postdef_dispensables = function(S) {
    var last_was_definition, remark;
    ...
    return $((function(_this) {
      return function(event, send) {
        ...
        if (select(event, ')', ':')) {
          ...
          return send(event);
        } else if (last_was_definition && select(event, '.', ['text', 'p'])) {
          type = event[0], name = event[1], text = event[2], meta = event[3];
          if (name === 'text') {
            debug('>>> 2');
            if (/^\n*$/.test(text)) {
              debug('>>> 3');
              return send(remark('drop', "blank `.text` after command definition", copy(meta)));
            } else {
              debug('>>> 4');
              return send(event);
            }
