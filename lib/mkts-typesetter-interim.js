(function() {
  ...
  this.pdf_from_md = function(source_route, handler) {
    ...
    input.pipe(D.$show()).pipe(HELPERS.TYPO.$fix_typography_for_tex()).pipe(this.$assemble_tex_events()).pipe(this.$filter_tex()).pipe(this.$insert_preamble()).pipe(this.$insert_postscript()).pipe(tex_output);
    return input.resume();
  };
