(function() {
  ...
  this.compile_options = function() {
    var cache_locator, cache_route, i, len, locators, options_home, options_locator, ref1, route, texinputs_routes;
  ...
  this.pdf_from_md = function(source_route, handler) {
    return step((function(_this) {
      return function*(resume) {
        ...
        input.pipe(TYPO.$fix_typography_for_tex()).pipe(TYPO.$show_mktsmd_events()).pipe(_this.MKTX.DOCUMENT.$open(state)).pipe(_this.MKTX.COMMAND.$new_page(state)).pipe(_this.MKTX.REGION.$keep_lines(state)).pipe(_this.MKTX.BLOCK.$heading(state)).pipe(_this.MKTX.BLOCK.$paragraph(state)).pipe(_this.MKTX.BLOCK.$hr(state)).pipe(_this.MKTX.INLINE.$code(state)).pipe(_this.MKTX.INLINE.$em_and_strong(state)).pipe(_this.MKTX.DOCUMENT.$close(state)).pipe(_this.$filter_tex()).pipe(tex_output);
        return input.resume();
      };
    })(this));
  };
