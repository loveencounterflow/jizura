(function() {
  var $, $async, CND, D, HELPERS, alert, badge, debug, echo, help, info, log, njs_fs, njs_path, options, rpr, step, suspend, urge, warn, whisper, ƒ,
    slice = [].slice;
  ...
  this.pdf_from_md = function(source_route, handler) {
    ...
    input.pipe(HELPERS.TYPO.$resolve_html_entities()).pipe(HELPERS.TYPO.$fix_typography_for_tex()).pipe(D.$show()).pipe(this.$assemble_tex_events()).pipe(this.$filter_tex()).pipe(this.$insert_preamble()).pipe(this.$insert_postscript()).pipe(tex_output);
    return input.resume();
  };
  ...
  this.$assemble_tex_events = function() {
    var add_newline_before_end, list_level, start_multicol_after, tag_stack, within_keeplines, within_multicol, within_pre;
    tag_stack = [];
