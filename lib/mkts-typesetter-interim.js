(function() {
  ...
  this.MKTX.COMMAND.$definition = (function(_this) {
    return function(S) {
      ...
      var identifier, track;
      track = MKTS.TRACKER.new_tracker('(:)', '[:]');
      identifier = null;
      S.definitions = {};
      return $(function(event, send) {
        var _, meta, target, type, within_definition;
        within_definition = track.within('(:)', '[:]');
        track(event);
        if (select(event, ['(', '['], ':')) {
          type = event[0], _ = event[1], identifier = event[2], meta = event[3];
          if (S.definitions[identifier] != null) {
            warn("re-defining command " + (rpr(identifier)));
          }
  ...
  this.MKTX.COMMAND.$definition_NG = (function(_this) {
    return function(S) {
      var track;
      track = MKTS.TRACKER.new_tracker('{definitions}');
      S.definitions = {};
      return $(function(event, send) {
        var identifier, meta, text, type, within_definition;
        within_definition = track.within('{definitions}');
        track(event);
        if (select(event, ['{', '}'], 'definitions')) {
          send(stamp(event));
          type = event[0], identifier = event[1], text = event[2], meta = event[3];
          if (type === '{') {
            return null;
          } else {
            return null;
          }
        } else if (within_definition && select(event, '.')) {
  ...
  this.MKTX.COMMAND.$expansion = (function(_this) {
    return function(S) {
      return $(function(event, send) {
        var _, definition, i, identifier, len, meta, sub_event, type;
        if (select(event, '!')) {
          type = event[0], identifier = event[1], _ = event[2], meta = event[3];
          if ((definition = S.definitions[identifier]) != null) {
  ...
  this.pdf_from_md = function(source_route, handler) {
    return step((function(_this) {
      return function*(resume) {
        ...
        input.pipe(MKTS.$fix_typography_for_tex(_this.options)).pipe(_this.MKTX.DOCUMENT.$begin(state)).pipe(_this.MKTX.DOCUMENT.$end(state)).pipe(_this.MKTX.MIXED.$raw(state)).pipe(_this.MKTX.COMMAND.$definition(state)).pipe(_this.MKTX.COMMAND.$expansion(state)).pipe(_this.MKTX.COMMAND.$new_page(state)).pipe(_this.MKTX.COMMAND.$comment(state)).pipe(_this.MKTX.COMMAND.$multi_column(state)).pipe(_this.MKTX.REGION.$multi_column(state)).pipe(_this.MKTX.REGION.$single_column(state)).pipe(_this.MKTX.REGION.$keep_lines(state)).pipe(_this.MKTX.REGION.$code(state)).pipe(_this.MKTX.BLOCK.$heading(state)).pipe(_this.MKTX.BLOCK.$hr(state)).pipe(_this.MKTX.INLINE.$code(state)).pipe(_this.MKTX.INLINE.$translate_i_and_b(state)).pipe(_this.MKTX.INLINE.$em_and_strong(state)).pipe(_this.MKTX.BLOCK.$paragraph(state)).pipe(_this.MKTX.CLEANUP.$remove_empty_texts(state)).pipe(MKTS.$close_dangling_open_tags(state)).pipe(MKTS.$show_mktsmd_events(state)).pipe(MKTS.$write_mktscript(state)).pipe(MKTS.$show_unhandled_tags(state)).pipe(_this.$filter_tex()).pipe(MKTS.$show_illegal_chrs(state)).pipe(tex_output);
        return input.resume();
      };
    })(this));
  };
