(function() {
  ...
  this.pdf_from_md = function(source_route, handler) {
    return step((function(_this) {
      return function*(resume) {
        ...
        input.pipe(TYPO.$fix_typography_for_tex(_this.options)).pipe(_this.MKTX.DOCUMENT.$begin(state)).pipe(_this.MKTX.COMMAND.$new_page(state)).pipe(_this.MKTX.REGION.$correct_p_tags_before_regions(state)).pipe(_this.MKTX.REGION.$multi_column(state)).pipe(_this.MKTX.REGION.$single_column(state)).pipe(_this.MKTX.REGION.$keep_lines(state)).pipe(_this.MKTX.REGION.$code(state)).pipe(_this.MKTX.BLOCK.$remove_empty_p_tags(state)).pipe(_this.MKTX.BLOCK.$heading(state)).pipe(_this.MKTX.BLOCK.$paragraph(state)).pipe(_this.MKTX.BLOCK.$hr(state)).pipe(_this.MKTX.INLINE.$code(state)).pipe(_this.MKTX.INLINE.$translate_i_and_b(state)).pipe(_this.MKTX.INLINE.$em_and_strong(state)).pipe(_this.MKTX.DOCUMENT.$end(state)).pipe(TYPO.$show_mktsmd_events(state)).pipe(TYPO.$write_mktscript(state)).pipe(_this.$show_unhandled_tags(state)).pipe(_this.$filter_tex()).pipe(tex_output);
        return input.resume();
      };
    })(this));
  };
        ...
        if (TYPO.isa(event, ['{', '}'], 'multi-column')) {
          ...
          if (type === '{') {
            if (!within_multi_column) {
              send(_this.MKTX.REGION._begin_multi_column());
              S.within_multi_column = true;
            } else {
              whisper("ignored " + type + name);
            }
          } else {
            if (within_multi_column) {
              send(_this.MKTX.REGION._end_multi_column());
              S.within_multi_column = false;
            } else {
              whisper("ignored " + type + name);
            }
          }
  ...
  this.MKTX.REGION.$single_column = (function(_this) {
    return function(S) {
      ...
      var track_multi_column, track_single_column;
      track_multi_column = TYPO.new_area_observer('{multi-column}');
      track_single_column = TYPO.new_area_observer('{single-column}');
      return $(function(event, send) {
        var meta, name, text, type, within_multi_column, within_single_column;
        within_multi_column = track_multi_column(event);
        ...
        if (TYPO.isa(event, ['{', '}'], 'single-column')) {
          ...
          type = event[0], name = event[1], text = event[2], meta = event[3];
          if (type === '{') {
            if ((!within_single_column) && within_multi_column) {
              send(_this.MKTX.REGION._end_multi_column());
            ...
            debug('@893', within_single_column === S.within_single_column);
            debug('@893', within_multi_column === S.within_multi_column);
            if (within_single_column && within_multi_column) {
              send(_this.MKTX.REGION._begin_multi_column());
            } else {
              whisper("ignored " + type + name);
      ...
      return $(function(event, send) {
        ...
        if (TYPO.isa(event, '.', 'text')) {
          ...
          if (type === '{') {
            ...
            S.within_pre = false;
            return S.just_closed_keeplines = true;
          }
        } else {
          return send(event);
        }
  ...
  this.MKTX.BLOCK.$remove_empty_p_tags = (function(_this) {
    return function(S) {
      ...
      return $(function(event, send) {
        if (TYPO.isa(event, '.', 'p')) {
          if (last_was_p) {
            return whisper("ignoring empty `p` tag");
          } else {
            last_was_p = true;
            return send(event);
          }
  ...
  this.MKTX.BLOCK.$heading = (function(_this) {
    return function(S) {
      var restart_multicols, track_multi_column;
        ...
        if (TYPO.isa(event, ['[', ']'], ['h1', 'h2', 'h3', 'h4', 'h5', 'h6'])) {
          ...
          if (type === '[') {
            if (within_multi_column && (name === 'h1' || name === 'h2')) {
              send(_this.MKTX.REGION._end_multi_column(meta));
              S.within_multi_column = false;
            ...
            if (restart_multicols) {
              send(_this.MKTX.REGION._begin_multi_column(meta));
              S.within_multi_column = true;
    ...
    return function(S) {
      return $(function(event, send) {
        ...
        if (TYPO.isa(event, '.', 'p')) {
          ...
          if (S.within_pre || S.within_keeplines) {
            send(_this.stamp(event));
            send(['text', '\n\n']);
          } else {
            send(_this.stamp(event));
          ...
          return S.just_closed_keeplines = false;
        } else {
          return send(event);
        }
      });
    ...
    return $((function(_this) {
      return function(event, send) {
        ...
        if ((ref1 = event[0]) === 'tex' || ref1 === 'text') {
          ...
          event_tex = TYPO.fix_typography_for_tex(rpr(event), _this.options);
