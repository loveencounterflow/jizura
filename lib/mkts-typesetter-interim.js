    ...
    return D.run((function(_this) {
      return function() {
        return step(function*(resume) {
          ...
          input.pipe(MKTS.$fix_typography_for_tex(_this.options)).pipe(_this.MKTX.DOCUMENT.$begin(S)).pipe(_this.MKTX.DOCUMENT.$end(S)).pipe(_this.MKTX.MIXED.$raw(S)).pipe(_this.MKTX.COMMAND.$do(S)).pipe(_this.MKTX.COMMAND.$expansion(S)).pipe(_this.MKTX.COMMAND.$new_page(S)).pipe(_this.MKTX.COMMAND.$comment(S)).pipe(_this.MKTX.COMMAND.$multi_column(S)).pipe(_this.MKTX.REGION.$multi_column(S)).pipe(_this.MKTX.REGION.$single_column(S)).pipe(_this.MKTX.REGION.$keep_lines(S)).pipe(_this.MKTX.REGION.$code(S)).pipe(_this.MKTX.BLOCK.$heading(S)).pipe(_this.MKTX.BLOCK.$hr(S)).pipe(_this.MKTX.BLOCK.$unordered_list(S)).pipe(_this.MKTX.INLINE.$code(S)).pipe(_this.MKTX.INLINE.$translate_i_and_b(S)).pipe(_this.MKTX.INLINE.$em_and_strong(S)).pipe(_this.MKTX.BLOCK.$paragraph(S)).pipe(_this.MKTX.CLEANUP.$remove_empty_texts(S)).pipe(MKTS.$close_dangling_open_tags(S)).pipe(MKTS.$show_mktsmd_events(S)).pipe(MKTS.$write_mktscript(S)).pipe(MKTS.$show_unhandled_tags(S)).pipe(_this.$filter_tex()).pipe(MKTS.$show_illegal_chrs(S)).pipe(tex_output);
          return input.resume();
        });
      };
    })(this), (function(_this) {
