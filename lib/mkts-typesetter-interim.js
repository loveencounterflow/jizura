(function() {
  ...
  this.pdf_from_md = function(source_route, handler) {
    ...
    input.pipe(TYPO.$resolve_html_entities()).pipe(TYPO.$fix_typography_for_tex()).pipe(D.$show()).pipe(this.$assemble_tex_events()).pipe(this.$filter_tex()).pipe(this.$insert_preamble(layout_info)).pipe(this.$insert_postscript()).pipe(tex_output);
    return input.resume();
  };
