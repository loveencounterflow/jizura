(function() {
  ...
  this.MKTX.REGION.$multi_column = (function(_this) {
    return function(S) {
      ...
      ref1 = MKTS.new_area_observer('multi-column'), track = ref1[0], within = ref1[1];
      return $(function(event, send) {
  ...
  this.MKTX.REGION.$keep_lines = (function(_this) {
    return function(S) {
      var ref1, track, within;
      ref1 = MKTS.new_area_observer('keep-lines'), track = ref1[0], within = ref1[1];
      return $(function(event, send) {
  ...
  this.MKTX.REGION.$code = (function(_this) {
    return function(S) {
      ...
      var ref1, track, within;
      ref1 = MKTS.new_area_observer('{code}'), track = ref1[0], within = ref1[1];
      return $(function(event, send) {
  ...
  this.MKTX.BLOCK.$paragraph = (function(_this) {
    return function(S) {
      ...
      ref1 = MKTS.new_area_observer('{code}'), track_1 = ref1[0], within_1 = ref1[1];
      ref2 = MKTS.new_area_observer('keep-lines'), track_2 = ref2[0], within_2 = ref2[1];
      return $(function(event, send) {
  ...
  this.pdf_from_md = function(source_route, handler) {
    return step((function(_this) {
      return function*(resume) {
        ...
        input.pipe(MKTS.$fix_typography_for_tex(_this.options)).pipe(_this.MKTX.DOCUMENT.$begin(state)).pipe(_this.MKTX.COMMAND.$new_page(state)).pipe(_this.MKTX.REGION.$correct_p_tags_before_regions(state)).pipe(_this.MKTX.REGION.$multi_column(state)).pipe(_this.MKTX.REGION.$single_column(state)).pipe(_this.MKTX.REGION.$keep_lines(state)).pipe(_this.MKTX.REGION.$code(state)).pipe(_this.MKTX.BLOCK.$remove_empty_p_tags(state)).pipe(_this.MKTX.BLOCK.$heading(state)).pipe(_this.MKTX.BLOCK.$paragraph(state)).pipe(_this.MKTX.BLOCK.$hr(state)).pipe(_this.MKTX.INLINE.$code(state)).pipe(_this.MKTX.INLINE.$translate_i_and_b(state)).pipe(_this.MKTX.INLINE.$em_and_strong(state)).pipe(_this.MKTX.DOCUMENT.$end(state)).pipe(MKTS.$show_mktsmd_events(state)).pipe(MKTS.$write_mktscript(state)).pipe(_this.$show_unhandled_tags(state)).pipe(_this.$filter_tex()).pipe(tex_output);
        return input.resume();
      };
    })(this));
  };
