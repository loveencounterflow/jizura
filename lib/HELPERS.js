(function() {
  var $, $async, ASYNC, CND, D, Markdown_parser, XNCHR, alert, badge, debug, echo, get_parse_open_tag_method, help, info, log, new_md_inline_plugin, njs_fs, njs_path, rpr, urge, warn, whisper,
  ...
  this.new_layout_info = function(options, source_route) {
    var R, aux_locator, content_locator, content_name, job_name, master_ext, master_locator, master_name, pdf_locator, source_home, source_locator, source_name, texinputs_value, xelatex_command;
  ...
  get_parse_open_tag_method = function() {
    var Parser, get_message, parser;
    ...
    return function(source) {
      ...
      return ['begin', cn['tagName'], (ref = cn['attrs'][0]) != null ? ref : {}];
    };
  };

  this.TYPO._parse_html_open_tag = get_parse_open_tag_method();
  ...
  this.TYPO.$rewrite_markdownit_tokens = function(S) {
    var is_first, last_map, unknown_tokens;
    unknown_tokens = [];
    is_first = true;
    last_map = [0, 0];
    return $((function(_this) {
      return function(token, send, end) {
        var atributes, map, meta, name, position, ref, type;
        if (token != null) {
          if (is_first) {
            is_first = false;
            send(['<', 'document', null, {}]);
          ...
          if (S.has_ended) {
            ...
            switch (type) {
              ...
              case 'html_block':
                ...
                break;
              case 'html_inline':
                ref = _this._parse_html_tag(token['content']), position = ref[0], name = ref[1], atributes = ref[2];
                switch (position) {
                  case 'begin':
                    send(['(', name, atributes, meta]);
                    break;
                  case 'end':
                    send([')', name, null, meta]);
                    break;
  ...
  this.TYPO.create_mdreadstream = function(md_source, settings) {
    ...
    R.pause();
    confluence.pipe(this.$flatten_tokens(state)).pipe(this.$rewrite_markdownit_tokens(state)).pipe(this.$preprocess_commands(state)).pipe(this.$process_end_command(state)).pipe(this.$preprocess_regions(state)).pipe(this.$close_dangling_open_tags(state)).pipe(R);
