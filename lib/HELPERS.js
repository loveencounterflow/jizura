(function() {
  ...
  this.write_pdf = function(layout_info, handler) {
    var aux_locator, count, digest, i, idx, job_name, last_digest, master_locator, parameters, pdf_from_tex, pdf_locator, ref, source_home, texinputs_value, xelatex_command;
  ...
  this.TYPO.$preprocess_keeplines_regions = function(S) {
    var ignore_next_close_p, next_text_starts_p, within_keep_lines, within_p;
    within_keep_lines = false;
    within_p = false;
    ignore_next_close_p = false;
    next_text_starts_p = false;
    return $((function(_this) {
      return function(event, send) {
        var meta, name, text, type;
        type = event[0], name = event[1], text = event[2], meta = event[3];
        if (_this.isa(event, '.', 'text')) {
          ...
          if (within_keep_lines) {
            ...
            if (ignore_next_close_p) {
              next_text_starts_p = true;
            } else {
              send(event);
            }
            ignore_next_close_p = false;
          }
        } else if (_this.isa(event, '{', 'keep-lines')) {
          within_keep_lines = true;
          if (within_p) {
