(function() {
  ...
  this.TYPO._get_opposite_fence = function(fence) {
    var R;
  ...
  this.TYPO.$update_meta = function(S) {
    var last_meta, tag_stack;
    tag_stack = [];
    last_meta = null;
    return D.$observe((function(_this) {
      return function(event) {
        var key, meta, name, text, type, value;
        type = event[0], name = event[1], text = event[2], meta = event[3];
        if (last_meta != null) {
          for (key in last_meta) {
            value = last_meta[key];
            if (meta[key] === void 0) {
              meta[key] = last_meta[key];
            }
          }
        }
        last_meta = meta;
        if (_this.isa(event, ['{', '[', '('])) {
          key = type + name + (_this._get_opposite_fence(type));
          return meta[key] = true;
        } else if (_this.isa(event, ['}', ']', ')'])) {
          key = (_this._get_opposite_fence(type)) + name + type;
  ...
  this.TYPO.$show_meta = function(S) {
    return $((function(_this) {
      return function(event, send) {
        var _, i, key, keys, len, meta, ref, signals, signals_rpr;
        if (!_this.isa(event, ['.', 'text', 'tex'])) {
          _ = event[0], _ = event[1], _ = event[2], meta = event[3];
          signals = [];
          ...
          info(signals_rpr);
          signals_rpr = _this.fix_typography_for_tex(signals_rpr, S.options);
          send(event);
          return send(['tex', "\\mktsComment{" + signals_rpr + "}"]);
        } else {
          return send(event);
        }
      };
  ...
  this.TYPO.create_mdreadstream = function(md_source, settings) {
    ...
    })(this)()).pipe(this.$rewrite_markdownit_tokens(state)).pipe(this.$preprocess_commands(state)).pipe(this.$process_end_command(state)).pipe(this.$preprocess_regions(state)).pipe(this.$close_dangling_open_tags(state)).pipe(this.$update_meta(state)).pipe(R);
