(function() {
  ...
  this.TYPO.new_area_observer = function(pattern) {
    var _, area_name, closing_fence, match, opening_fence, toggle;
    if (match === '*') {
      throw new Error("area observer pattern `*` (star) not yet implemented");
    } else {
      match = pattern.match(/^(.)(.+)(.)$/);
      if (!match) {
        throw new Error("illegal area observer pattern " + (rpr(pattern)));
      }
      _ = match[0], opening_fence = match[1], area_name = match[2], closing_fence = match[3];
      if (closing_fence !== this._get_opposite_fence(opening_fence, null)) {
        throw new Error("illegal fences in area observer pattern " + (rpr(pattern)) + ":\n" + (rpr(opening_fence)) + ", " + (rpr(closing_fence)));
      }
      toggle = false;
      return (function(_this) {
        return function(event) {
          ...
          if (event != null) {
            type = event[0], name = event[1], text = event[2], meta = event[3];
            if (_this.isa(event, [opening_fence, closing_fence], area_name)) {
