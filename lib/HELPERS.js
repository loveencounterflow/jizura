(function() {
        ...
        if (event != null) {
          ...
          for (i = 0, len = lines.length; i < len; i++) {
            ...
            if ((match = line.match(opening_pattern)) != null) {
              _this._flush_text_collector(send, collector, meta);
              region_name = match[1];
              region_stack.push(region_name);

              /* TAINT use `meta`? */
              send([
                '{', region_name, null, _this._copy(meta, {
                  block: true
                })
              ]);
  ...
  this.TYPO.isa = function(event, type, name) {
    return (event[0] === type) && (event[1] === name);
  };
  ...
  this.TYPO._flush_text_collector = function(send, collector, meta) {
    if (collector.length > 0) {
      send(['.', 'text', collector.join(''), this._copy(meta)]);
      collector.length = 0;
    }
    return null;
  };
  ...
  this.TYPO.create_mdreadstream = function(md_source, settings) {
    ...
    R.pause();
    confluence.pipe(this.$flatten_tokens()).pipe(this.$rewrite_markdownit_tokens()).pipe(this.$preprocess_regions()).pipe(this.$preprocess_commands()).pipe(this.$add_lookahead()).pipe(R);
