(function() {
  ...
  this.TYPO._new_markdown_parser = function() {
    ...
    R.use(user_plugin);
    R.use(require('markdown-it-container'), 'keeplines', {
  ...
  this.TYPO.create_html_readstream_from_md = function(text, settings) {
    var $consolidate_texts, $remove_block_tags_from_keeplines, R, html, html_parser, md_parser;
    if (settings != null) {
    ...
    html_parser.end();
    $remove_block_tags_from_keeplines = (function(_this) {
      return function() {
        ...
        return $(function(event, send) {
          ...
          if (type === 'open-tag' && tag === 'keeplines') {
            within_keeplines = true;
            return send(event);
          }
          if (type === 'close-tag' && tag === 'keeplines') {
            within_keeplines = false;
            return send(event);
          }
          if (within_keeplines) {
    ...
    $consolidate_texts = (function(_this) {
      return function() {
    ...
    R = R.pipe($remove_block_tags_from_keeplines()).pipe($consolidate_texts());
    return R;
  };
