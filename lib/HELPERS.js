(function() {
  var $, $async, ASYNC, CND, D, Markdown_parser, XNCHR, alert, badge, debug, echo, get_parse_html_methods, help, info, log, misfit, new_md_inline_plugin, njs_fs, njs_path, parse_methods, rpr, urge, warn, whisper,
  ...
  this.TYPO.$reinject_html_blocks = function(S) {
  ...
  this.TYPO.new_area_observer = function() {
    ...
    for (i = 0, len = area_names.length; i < len; i++) {
      ...
      if ((regular_closing_fence = this._get_opposite_fence(opening_fence, null)) != null) {
  ...
  this.TYPO.create_mdreadstream = function(md_source, settings) {
    ...
    confluence.pipe(this.$flatten_tokens(state)).pipe(this.$reinject_html_blocks(state)).pipe(this.$rewrite_markdownit_tokens(state)).pipe(this.$preprocess_commands(state)).pipe(this.$process_end_command(state)).pipe(this.$preprocess_regions(state)).pipe(this.$close_dangling_open_tags(state)).pipe(R);
