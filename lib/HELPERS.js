(function() {
  ...
  this.TYPO._$remove_superfluous_tags = function() {
    var skip_next_closing_anchor, skip_next_closing_hr, skip_next_text;
    skip_next_text = false;
    skip_next_closing_anchor = false;
    skip_next_closing_hr = false;
    return $((function(_this) {
      return function(event, send) {
        var attributes, tag_name, type;
        ...
        if (type === 'text') {
          ...
          if (tag_name === 'mkts-mark') {
            return;
          }
          if (skip_next_closing_anchor && tag_name === 'a') {
            skip_next_closing_anchor = false;
            return;
          }
          if (skip_next_closing_hr && tag_name === 'hr') {
            skip_next_closing_hr = false;
            return;
          }
        } else if (type === 'open-tag') {
          if (tag_name === 'a' && attributes['class'] === 'footnote-backref') {
            skip_next_text = true;
            skip_next_closing_anchor = true;
            return;
          }
          if (tag_name === 'hr' && attributes['class'] === 'footnotes-sep') {
            skip_next_closing_hr = true;
            return;
          }
        }
        return send(event);
  ...
  this.TYPO.create_html_readstream_from_md = function(md_source, settings) {
    ...
    html_parser.end();
    R = R.pipe(this._$remove_superfluous_tags()).pipe(this._$add_regions()).pipe(this._$add_commands()).pipe(this._$remove_block_tags_from_keeplines()).pipe(this._$consolidate_texts());
