(function() {
  ...
  this.TYPO._$add_regions = function() {
    var closing_pattern, opening_pattern, stack;
    stack = [];
    opening_pattern = /^@@@[\x20\xa0]+(\S.+)(?:\n|$)/;
    closing_pattern = /(^|\n)@@@\s*$/;
    return $((function(_this) {
      return function(event, send, end) {
        var closing_match, name, opening_match, text, type;
        if (event != null) {
          ...
          if (type === 'text') {
            ...
            if (opening_match != null) {
              ...
              text = text.slice(opening_match[0].length);
              stack.push(name);
              send(['start-region', name]);
            }
            if (closing_match != null) {
              text = text.slice(0, text.length - closing_match[0].length);
            }
            send(['text', text]);
            if (closing_match != null) {
              if (stack.length > 0) {
  ...
  this.TYPO.create_html_readstream_from_md = function(text, settings) {
    ...
    html_parser.end();
    R = R.pipe(this._$add_regions()).pipe(this._$add_commands()).pipe(this._$remove_block_tags_from_keeplines()).pipe(this._$consolidate_texts());
    return R;
  };
