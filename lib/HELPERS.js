(function() {
  ...
  this.TYPO.$rewrite_markdownit_tokens = function(S) {
    ...
    return $((function(_this) {
      return function(token, send, end) {
        var extra, language_name, map, meta, name, position, ref, type;
        _send = send;
        if (token != null) {
          if (is_first) {
            is_first = false;
          ...
          if (S.has_ended) {
            ...
            switch (type) {
              ...
              case 'fence':
                ...
                break;
              case 'html_inline':
                ref = _this._parse_html_tag(token['content']), position = ref[0], name = ref[1], extra = ref[2];
                switch (position) {
                  case 'comment':
  ...
  this.TYPO.$exp_echo_mktscript = function(S) {
    var indentation, tag_stack, write;
    indentation = '';
    tag_stack = [];
    write = process.stdout.write.bind(process.stdout);
    return D.$observe(function(event, has_ended) {
      var meta, name, text, text_rpr, type;
      if (event != null) {
        type = event[0], name = event[1], text = event[2], meta = event[3];
        switch (type) {
          case 'tex':
