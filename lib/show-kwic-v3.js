(function() {
  ...
  this.show_kwic_v3 = function(db) {
    return step((function(_this) {
      return function*(resume) {
        var $align_affixes, $count_lineup_lengths, $exclude_gaiji, $include_sample, $insert_hr, $reorder_phrase, $show, $transform_v3, db_route, factor_sample, glyph_sample, include, input_v3, lineup_left_count, lineup_right_count, query_v3, ranks;
        db_route = join(__dirname, '../../jizura-datasources/data/leveldb-v2');
        if (db == null) {
          db = HOLLERITH.new_db(db_route, {
            create: false
        ...
        $count_lineup_lengths = function() {
          ...
          count = 0;
          return $(function(event, send, end) {
            var _, count_txt, glyph, i, infix, length, lineup, lineup_length, prefix, ref, ref1, sortcode, suffix;
            if (event != null) {
        ...
        $align_affixes = function() {
          return $(function(event, send) {
            var _, glyph, infix, post_suffix, pre_prefix, prefix, sortcode, suffix;
            if (CND.isa_list(event)) {
              glyph = event[0], sortcode = event[1];
              _ = sortcode[0], infix = sortcode[1], suffix = sortcode[2], prefix = sortcode[3];
              pre_prefix = [];
              ...
              while (suffix.length > lineup_right_count) {
                pre_prefix.unshift(suffix.pop());
              }
              while (prefix.length > lineup_left_count) {
                post_suffix.push(prefix.shift());
              }
              while (!(prefix.length >= lineup_left_count - pre_prefix.length)) {
                prefix.unshift('\u3000');
              }
              while (!(suffix.length >= lineup_right_count - post_suffix.length)) {
                suffix.push('\u3000');
              }
              return send([glyph, [pre_prefix, prefix, infix, suffix, post_suffix]]);
            } else {
              return send(event);
            }
          });
        ...
        $transform_v3 = function() {
          return D.combine([$reorder_phrase(), $exclude_gaiji(), $include_sample(), $insert_hr(), $count_lineup_lengths(), $align_affixes()]);
        };
        $show = function() {
          return D.$observe(function(event) {
            var glyph, infix, lineup, post_suffix, pre_prefix, prefix, suffix;
            if (CND.isa_list(event)) {
              glyph = event[0], lineup = event[1];
              pre_prefix = lineup[0], prefix = lineup[1], infix = lineup[2], suffix = lineup[3], post_suffix = lineup[4];
