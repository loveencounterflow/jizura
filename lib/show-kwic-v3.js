(function() {
  ...
  this.show_kwic_v3 = function(db) {
    return step((function(_this) {
      return function*(resume) {
        var $_XXX_sort, $align_affixes, $count_glyphs_etc, $count_lineup_lengths, $exclude_gaiji, $include_sample, $insert_hr, $insert_keeplines, $reorder_phrase, $show, $transform_v3, db_route, factor_sample, glyph_sample, include, input_v3, prefix_max_length, query_v3, ranks, suffix_max_length, window_width;
        db_route = join(__dirname, '../../jizura-datasources/data/leveldb-v2');
        if (db == null) {
          db = HOLLERITH.new_db(db_route, {
            create: false
...
=======
            var _, glyph, infix, left, overall_length, postsuffix, prefix, preprefix, right, sortcode, suffix;
            if (CND.isa_list(event)) {
              glyph = event[0], sortcode = event[1];
              _ = sortcode[0], infix = sortcode[1], suffix = sortcode[2], prefix = sortcode[3];
              ...
              left = slice.call(preprefix).concat(slice.call(prefix));
              right = slice.call(suffix).concat(slice.call(postsuffix));
              return send([glyph, [sortcode, left, infix, right]]);
>>>>>>> e3d595d7d692ecea4549a589798c7e3c5028a236
        ...
        $show = function() {
          ...
          return D.$observe(function(event) {
            ...
            if (CND.isa_list(event)) {
              glyph = event[0], sortcode = event[1];
              _ = sortcode[0], prefix = sortcode[1], infix = sortcode[2], suffix = sortcode[3];
              prefix = prefix.join('');
              suffix = suffix.join('');
...
>>>>>>> e3d595d7d692ecea4549a589798c7e3c5028a236
        $transform_v3 = function() {
          return D.combine([$reorder_phrase(), $exclude_gaiji(), $include_sample(), $count_lineup_lengths(), $_XXX_sort(), $insert_keeplines(), $align_affixes(), $count_glyphs_etc(), $show()]);
        };
        query_v3 = {
