...
(function() {
  var $, $async, $count_lineup_lengths, $exclude_gaiji, $include_sample, $reorder_phrase, $transform_v3, $write_glyphs, $write_glyphs_description, $write_stats, ASYNC, CHR, CND, D, HOLLERITH, KWIC, TEXT, after, alert, badge, debug, echo, eventually, every, help, immediately, info, join, log, new_db, njs_fs, njs_path, options, repeat_immediately, rpr, step, suspend, urge, warn, whisper, ƒ,
    slice = [].slice;
  ...
  $write_stats = (function(_this) {
    return function(S) {
      var factor_pairs, glyphs, infixes, line_count, lineup_count, output;
      glyphs = new Set();
      infixes = new Set();
      ...
      return D.$observe(function(event, has_ended) {
        var entry, factor_pair, glyph, glyph_count, glyph_count_txt, i, infix, j, key, last_infix, len, len1, line, prefix, ref, ref1, separator, suffix, target;
        if (event != null) {
          if (CND.isa_list(event)) {
            glyph = event[0], prefix = event[1], infix = event[2], suffix = event[3];
        ...
        if (has_ended) {
          ...
          factor_pairs.sort(function(a, b) {
            var a_glyphs, a_infix, a_infix_idx, a_pair, b_glyphs, b_infix, b_infix_idx, b_pair, ref, ref1;
            (ref = a[0], a_infix = ref[0], a_pair = ref[1]), a_glyphs = a[1];
            (ref1 = b[0], b_infix = ref1[0], b_pair = ref1[1]), b_glyphs = b[1];
          ...
          for (j = 0, len1 = factor_pairs.length; j < len1; j++) {
            ref = factor_pairs[j], (ref1 = ref[0], infix = ref1[0], factor_pair = ref1[1]), glyphs = ref[1];
            if (last_infix !== infix) {
              output.write("——." + infix + ".——\n");
            }
            last_infix = infix;
  ...
  $write_glyphs = (function(_this) {
    return function(S) {
      var is_first, line_count, output;
  ...
  $transform_v3 = (function(_this) {
    return function(S) {
      return D.combine([
        $reorder_phrase(S), $exclude_gaiji(S), $include_sample(S), $write_stats(S), $write_glyphs(S), $write_glyphs_description(S), D.$on_end(function() {
          if (S.handler != null) {
            return S.handler(null);
          }
        })
