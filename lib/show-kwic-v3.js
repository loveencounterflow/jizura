  ...
  this.show_kwic_v3 = function(db) {
    return step((function(_this) {
      return function*(resume) {
        var $count_glyphs_etc, $count_lineup_lengths, $exclude_gaiji, $include_sample, $insert_hr, $insert_many_keeplines, $insert_single_keeplines, $reorder_phrase, $show, $transform_v3, db_route, factor_sample, glyph_sample, include, input_v3, query_v3, ranks;
        db_route = join(__dirname, '../../jizura-datasources/data/leveldb-v2');
        if (db == null) {
          db = HOLLERITH.new_db(db_route, {
            create: false
        ...
        $insert_single_keeplines = function() {
        ...
        $count_glyphs_etc = function() {
          var factor_pairs, glyphs, lineup_count;
          glyphs = new Set();
          factor_pairs = {};
          lineup_count = 0;
          return D.$observe(function(event, has_ended) {
            var count, factor_pair, glyph, i, infix, key, len, prefix, ref, ref1, ref2, results, suffix;
            if (event != null) {
              if (CND.isa_list(event)) {
                glyph = event[0], prefix = event[1], infix = event[2], suffix = event[3];
                prefix = prefix.trim();
                ...
                if (prefix.length > 0) {
                  ...
                  factor_pairs[key] = ((ref = factor_pairs[key]) != null ? ref : 0) + 1;
                }
                if (suffix.length > 0) {
                  suffix = Array.from(suffix);
                  key = '\u3000' + infix + suffix[0];
                  factor_pairs[key] = ((ref1 = factor_pairs[key]) != null ? ref1 : 0) + 1;
                }
                glyphs.add(glyph);
                lineup_count += +1;
              }
            ...
            if (has_ended) {
              ...
              factor_pairs = (function() {
                ...
                for (factor_pair in factor_pairs) {
                  count = factor_pairs[factor_pair];
                  results.push([factor_pair, count]);
                }
                return results;
              })();
              ...
              for (i = 0, len = factor_pairs.length; i < len; i++) {
                ref2 = factor_pairs[i], factor_pair = ref2[0], count = ref2[1];
                results.push(urge(factor_pair, count));
              }
              return results;
            }
          });
        ...
        $transform_v3 = function() {
          return D.combine([$reorder_phrase(), $exclude_gaiji(), $include_sample(), $insert_single_keeplines(), $count_glyphs_etc(), $show()]);
        };
        query_v3 = {
