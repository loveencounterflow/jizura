(function() {
  ...
  this.show_kwic_v3 = function(db) {
    return step((function(_this) {
      return function*(resume) {
        var $exclude_gaiji, $format_sortcode_v3, $include_sample, $reorder_phrase, $show, $transform_v3, db_route, factor_infos, factor_sample, glyph_sample, include, input_v3, query_v3, ranks;
        db_route = join(__dirname, '../../jizura-datasources/data/leveldb-v2');
        if (db == null) {
          db = HOLLERITH.new_db(db_route, {
            create: false
        ...
        $include_sample = function() {
          return D.$filter(function(arg) {
            ...
            if (glyph_sample != null) {
              if (glyph in glyph_sample) {
                return true;
              }
            }
            _ = sortcode[0], infix = sortcode[1], suffix = sortcode[2], prefix = sortcode[3];
            if (factor_sample) {
              if (infix in factor_sample) {
                return true;
              }
            }
            return false;
