(function() {
  ...
  this.$add_factorial_factors = function(factor_infos) {
     ...
     */
    var _, factor, factors, factors_by_factor, glyph_count, resolve_ics_to_factors, seen_glyphs;
    factors = new Set();
    ...
    seen_glyphs = new Set();
    glyph_count = 0;
    resolve_ics_to_factors = (function(_this) {
      return function(glyph) {
        var entry, ic, k, len, ref, results, sub_entry, sub_ic;
        if (seen_glyphs.has(glyph)) {
          return;
        ...
        seen_glyphs.add(glyph);
        if ((entry = factors_by_factor[glyph]) != null) {
          ref = Array.from(entry.keys());
          results = [];
          for (k = 0, len = ref.length; k < len; k++) {
            ic = ref[k];
            resolve_ics_to_factors(ic);
            if ((sub_entry = factors_by_factor[ic]) != null) {
              results.push((function() {
                var l, len1, ref1, results1;
                ref1 = Array.from(sub_entry);
                results1 = [];
      ...
      return function(phrase, send, end) {
        ...
        if (phrase != null) {
          ...
          for (formula_idx = k = 0, len = obj.length; k < len; formula_idx = ++k) {
            formula = obj[formula_idx];
            ics = IDLX.find_all_non_operators(formula);
            target = factors_by_factor[glyph] != null ? factors_by_factor[glyph] : factors_by_factor[glyph] = new Set();
            for (l = 0, len1 = ics.length; l < len1; l++) {
              ic = ics[l];
              target.add(XNCHR.as_uchr(ic));
            }
        ...
        if (end != null) {
          for (glyph in factors_by_factor) {
            resolve_ics_to_factors(glyph);
          }
          for (glyph in factors_by_factor) {
            entry = factors_by_factor[glyph];
            factors_by_factor[glyph] = Array.from(entry);
          }
          end();
        }
        return null;
  ...
  this.$add_lineup_postfixes = function() {
    return $((function(_this) {
