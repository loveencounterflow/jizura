(function() {
  ...
  this.$add_factorial_factors = function(factor_infos) {
    ...
    return $((function(_this) {
      return function(phrase, send, end) {
        var entry, factors_by_factor_json, factors_json, factors_route, formula, formula_idx, formulas_route, glyph, ic, ics, k, l, len, len1, len2, len3, m, n, obj, prd, ref;
        if (phrase != null) {
          send(phrase);
          glyph = phrase[0], prd = phrase[1], obj = phrase[2];
          if (prd !== 'formula') {
        ...
        if (end != null) {
          ...
          formulas_route = njs_path.resolve(__dirname, '../../jizura-datasources/data/5-derivatives/formulas-by-glyphs.json');
          factors_route = njs_path.resolve(__dirname, '../../jizura-datasources/data/5-derivatives/factors.json');
          warn("write to " + formulas_route);
          njs_fs.writeFileSync(formulas_route, factors_by_factor_json);
          warn("write to " + factors_route);
          njs_fs.writeFileSync(factors_route, factors_json);
          ref = Array.from(factors.keys());
          for (l = 0, len1 = ref.length; l < len1; l++) {
            factor = ref[l];
            if ((entry = factors_by_factor[factor]) == null) {
              whisper("no ICs for factor " + factor);
              continue;
            }
            for (m = 0, len2 = entry.length; m < len2; m++) {
              ics = entry[m];
              for (n = 0, len3 = ics.length; n < len3; n++) {
                ic = ics[n];
                if (!factors.has(ic)) {
