(function() {
  var $, $async, CND, D, HOLLERITH, KWIC, XNCHR, alert, badge, debug, echo, find_duplicated_guides, help, info, join, log, njs_fs, njs_path, options, rpr, step, suspend, urge, warn, whisper, ƒ,
  ...
  this.copy_jizura_db = function() {
    ...
    return step((function(_this) {
      return function*(resume) {
        ...
        return input.pipe(_this.v1_$split_so_bkey()).pipe(_this.$show_progress(1e4)).pipe(_this.$keep_small_sample()).pipe(_this.$throw_out_pods()).pipe(_this.$remove_duplicate_kana_readings()).pipe(_this.$cast_types(ds_options)).pipe(_this.$collect_lists()).pipe(_this.$compact_lists()).pipe(_this.$add_version_to_kwic_v1()).pipe(_this.$add_kwic_v2()).pipe(_this.$add_kwic_v3(factor_infos)).pipe(_this.$add_kwic_v3_wrapped_lineups(factor_infos)).pipe(_this.$add_guide_pairs(factor_infos)).pipe(_this.$add_factor_membership(factor_infos)).pipe(_this.$add_sims()).pipe(D.$count(function(count) {
