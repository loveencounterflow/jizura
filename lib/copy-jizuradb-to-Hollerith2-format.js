(function() {
  ...
  this.$add_kwic_v3_subfactors = function(factor_infos) {
    ...
    resolve_ics = (function(_this) {
      return function(glyph) {
        var entry, ic, k, len, ref, results, sub_ic;
        seen_glyphs.add(glyph);
        ref = Array.from(ics_by_glyph[glyph].keys());
        results = [];
        for (k = 0, len = ref.length; k < len; k++) {
          ic = ref[k];
          if (seen_glyphs.has(ic)) {
            continue;
          }
          if ((entry = ics_by_glyph[ic]) != null) {
            results.push((function() {
              var l, len1, ref1, results1;
              ref1 = Array.from(entry);
              results1 = [];
              for (l = 0, len1 = ref1.length; l < len1; l++) {
                sub_ic = ref1[l];
                results1.push(null);
    ...
    return $((function(_this) {
      return function(phrase, send, end) {
        var formula, formula_idx, glyph, ic, ics, k, l, len, len1, obj, prd, target;
        if (phrase != null) {
          send(phrase);
          glyph = phrase[0], prd = phrase[1], obj = phrase[2];
          if (prd !== 'formula') {
          ...
          if (glyph_count > 1500) {
            echo(rpr(ics_by_glyph));
            throw new Error('23423647');
          }
        }
        if (end != null) {
          return end();
