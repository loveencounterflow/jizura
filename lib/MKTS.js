(function() {
  ...
  this._tex_escape_replacements = [[/\x01/g, '\x01\x02'], [/\x5c/g, '\x01\x01'], [/\{/g, '\\{'], [/\}/g, '\\}'], [/\$/g, '\\$'], [/\#/g, '\\#'], [/%/g, '\\%'], [/_/g, '\\_'], [/\^/g, '\\textasciicircum{}'], [/~/g, '\\textasciitilde{}'], [/&amp;/g, '\\&'], [/&quot;/g, '"'], [/'([^\s]+)’/g, '‘$1’'], [/&/g, '\\&'], [/\x01\x01/g, '\\textbackslash{}'], [/\x01\x02/g, '\x01']];

  this.escape_for_tex = function(text) {
    var R, i, idx, len, pattern, ref, ref1, replacement;
    R = text;
    debug('©0AdXZxx', rpr(text));
    ref = this._tex_escape_replacements;
    for (idx = i = 0, len = ref.length; i < len; idx = ++i) {
      ref1 = ref[idx], pattern = ref1[0], replacement = ref1[1];
      R = R.replace(pattern, replacement);
      if (idx === 0 || idx === 1 || idx === 15 || idx === 16) {
        debug('©xxxx ' + (idx + 40), rpr(R));
      }
    }
    return R;
  };
  ...
  this.resolve_html_entities = function(text) {
    var R;
    R = text;
    R = R.replace(/&lt;/g, '<');
    R = R.replace(/&gt;/g, '>');
    ...
    return function() {
      ...
      self = function(event) {
        ...
        for (pattern in ref) {
          state = ref[pattern];
          parts = state.parts;
          if (!_MKTS.isa.apply(_MKTS, [event].concat(slice.call(parts)))) {
            continue;
          }
  ...
  this.$_preprocess_regions = function(S) {
    var closing_pattern, collector, opening_pattern, region_stack, track;
    opening_pattern = /^@@@(\S.+)(\n|$)/;
    closing_pattern = /^@@@\s*(\n|$)/;
    collector = [];
    region_stack = [];
    track = this.TRACKER.new_tracker('(code)');
    return $((function(_this) {
      return function(event, send) {
        var i, len, line, lines, match, meta, name, region_name, text, type, within_code;
        within_code = track.within('(code)');
        track(event);
        type = event[0], name = event[1], text = event[2], meta = event[3];
        if ((!within_code) && (_this.isa(event, '.', 'text'))) {
  ...
  this.$_preprocess_XXXX = function(S) {
    var collector, left_meta_fence, pattern, repetitions, right_meta_fence, track;
    left_meta_fence = '<';
    right_meta_fence = '>';
    repetitions = 2;
    pattern = RegExp(left_meta_fence + "{" + repetitions + "}((?:\\\\" + right_meta_fence + "|[^" + right_meta_fence + "]|" + right_meta_fence + "{" + (repetitions - 1) + "}(?!" + right_meta_fence + "))*)" + right_meta_fence + "{" + repetitions + "}");
    collector = [];
    track = this.TRACKER.new_tracker('{code}', '(code)', '(latex)', '(latex)');
    return $((function(_this) {
      return function(event, send) {
        var command_name, i, is_command, last_idx, left_fence, len, meta, name, part, ref, ref1, ref2, right_fence, text, type, within_literal;
        within_literal = track.within('{code}', '(code)', '(latex)', '(latex)');
        track(event);
        type = event[0], name = event[1], text = event[2], meta = event[3];
        if ((!within_literal) && _this.isa(event, '.', 'text')) {
          is_command = true;
          ...
          for (i = 0, len = ref.length; i < len; i++) {
            ...
            if (is_command) {
              ...
              if ((left_fence != null) && (right_fence != null)) {
                ...
                send([left_fence, command_name, null, _this._copy(meta)]);
                send([right_fence, command_name, null, _this._copy(meta)]);
                ...
                command_name = part;
                send(['∆', command_name, null, _this._copy(meta)]);
              }
            } else {
              send([type, name, part, _this._copy(meta)]);
            }
          }
        } else {
          send(event);
  ...
  this.$_preprocess_commands = function(S) {
    var collector, pattern, track;
    pattern = /^∆∆∆(\S.+)(\n|$)/;
    collector = [];
    track = this.TRACKER.new_tracker('(code)');
    return $((function(_this) {
      return function(event, send) {
        var i, len, line, lines, match, meta, name, text, type, within_code;
        within_code = track.within('(code)');
        track(event);
        type = event[0], name = event[1], text = event[2], meta = event[3];
        if ((!within_code) && _this.isa(event, '.', 'text')) {
  ...
  this.$_close_dangling_open_tags = function(S) {
    var tag_stack;
    tag_stack = [];
    return $((function(_this) {
      return function(event, send) {
  ...
  this.isa = function(event, type, name) {
  ...
  this._copy = function(meta, overwrites) {
  ...
  this.create_mdreadstream = function(md_source, settings) {
    ...
    confluence.pipe(this.$_flatten_tokens(state)).pipe(this.$_reinject_html_blocks(state)).pipe(this.$_rewrite_markdownit_tokens(state)).pipe(this.$_preprocess_XXXX(state)).pipe(this.$_preprocess_commands(state)).pipe(this.$_process_end_command(state)).pipe(this.$_preprocess_regions(state)).pipe(this.$_close_dangling_open_tags(state)).pipe(R);
