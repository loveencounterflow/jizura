(function() {
  var $, CND, D, Markdown_parser, XNCHR, alert, badge, debug, echo, fences_rxcc, get_parse_html_methods, help, info, log, misfit, name_rx, new_md_inline_plugin, njs_fs, njs_path, parse_methods, rpr, tracker_pattern, urge, warn, whisper,
  ...
  this.fix_typography_for_tex = function(text, options, send) {
    var R, advance, chr, chr_info, command, fncr, glyph_styles, i, last_command, last_rsg, len, message, ref, ref1, ref2, ref3, remark, replacement, rsg, stretch, tex_command_by_rsgs, uchr;
    if (send == null) {
      send = null;
    }
    ...
    stretch = [];
    last_rsg = null;
    remark = send != null ? this._get_remark() : null;
  ...
  this._new_markdown_parser = function() {
    ...
    var R, feature_set, settings, user_handler, user_pattern, user_plugin;
    feature_set = 'zero';
    settings = {
      html: true,
      xhtmlOut: false,
  ...
  this.$_rewrite_markdownit_tokens = function(S) {
    var _send, is_first, last_map, remark, send_unknown, unknown_tokens;
    unknown_tokens = [];
    is_first = true;
    last_map = [0, 0];
    _send = null;
  ...
  this.$show_unhandled_tags = function(S) {
    return $((function(_this) {
      return function(event, send) {
        ...
        var event_tex, event_txt, first, last, meta, name, post, pre, ref, text, type;
        if ((ref = event[0]) === 'tex' || ref === 'text') {
          return send(event);
        } else if (_this.select(event, '.', 'text')) {
          return send(event);
        } else if (!_this.is_stamped(event)) {
          type = event[0], name = event[1], text = event[2], meta = event[3];
          if (text != null) {
            if (CND.isa_pod(text)) {
          ...
          if ((type === '.' || type === '!') || indexOf.call(_this.FENCES.xleft, type) >= 0) {
            first = type;
            last = name;
            pre = '█';
            post = '';
          ...
          event_tex = _this.fix_typography_for_tex(event_txt, S.options);
  ...
  this.$show_illegal_chrs = function(S) {
    return $(function(old_text, send) {
      ...
      new_text = old_text.replace(/[\x00-\x08\x0b\x0c\x0e-\x1f\x7f\ufffd-\uffff]/g, function($0) {
        ...
        /* TAINT use mkts command */
        warn("detected (probably) illegal character U+" + cid_hex);
  ...
  this.XXX_comment_by_ids = new Map();

  this.XXX_id_by_comments = new Map();

  this.XXX_raw_content_by_ids = new Map();

  this.XXX_raw_id_by_contents = new Map();

  this.XXX_command_by_ids = new Map();

  this.XXX_id_by_commands = new Map();


  /* TAINT new: */

  this.XXX_registry = new Map([['action', new Map()], ['comment', new Map()], ['do', new Map()], ['raw', new Map()]]);

  this.XXX_html_comment_pattern = /(?:(^|[^\\])<!---->)|(?:(^|[^\\])<!--([\s\S]*?[^\\])-->)/g;

  this.XXX_do_bracketed_pattern = /(?:(^|[^\\])<<\{do>><<do\}>>)|(?:(^|[^\\])<<\{do>>(.*?[^\\])<<do\}>>)/g;

  this.XXX_raw_bracketed_pattern = /(?:(^|[^\\])<<\(raw>><<raw\)>>)|(?:(^|[^\\])<<\(raw>>(.*?[^\\])<<raw\)>>)/g;

  this.XXX_raw_heredoc_pattern = /(^|[^\\])<<!raw:([^\s>]*)>>(.*?)\2/g;

  this.XXX_raw_id_pattern = /\x11([0-9]+)\x13/g;

  this.XXX_html_comment_id_pattern = /\x14([0-9]+)\x13/g;

  this.XXX_command_id_pattern = /\x12([0-9]+)\x13/g;

  this.XXX_command_pattern = /(^|[^\\])(<<([!{[(]?)([^\s!{[()\]>}]+?)([)\]}]?)>>)/g;

  this.XXX_escape_html_comments_raw_spans_and_commands = function(text) {
    var R;
    R = text;
    R = this.XXX_escape_escape_chrs(R);
    R = R.replace(this.XXX_html_comment_pattern, (function(_this) {
      return function(_, $1, $2, $3) {
        var id, raw_content;
        if ($1 == null) {
          $1 = '';
        }
        ...
        $1 += $2;
        raw_content = $3 != null ? $3 : '';
        id = _this.XXX_raw_id_from_content('comment', raw_content.trim());
        return $1 + "\x14" + id + "\x13";
      };
    })(this));
    R = R.replace(this.XXX_do_bracketed_pattern, (function(_this) {
      return function(_, $1, $2, $3) {
        var id, raw_content;
        if ($1 == null) {
          $1 = '';
        ...
        $1 += $2;
        raw_content = $3 != null ? $3 : '';
        id = _this.XXX_raw_id_from_content('raw', raw_content);
        return $1 + "\x11" + id + "\x13";
      };
    })(this));
    R = R.replace(this.XXX_raw_bracketed_pattern, (function(_this) {
      return function(_, $1, $2, $3) {
        var id, raw_content;
        if ($1 == null) {
          $1 = '';
    ...
    R = R.replace(this.XXX_raw_heredoc_pattern, (function(_this) {
      return function(_, $1, $2, $3) {
        var id, raw_content;
        raw_content = $3 != null ? $3 : '';
        id = _this.XXX_raw_id_from_content('raw', raw_content);
    ...
    R = R.replace(this.XXX_command_pattern, (function(_this) {
      return function(_, $1, $2, $3, $4, $5) {
        var id, parsed_content, raw_content;
        raw_content = $2;
        parsed_content = [$3, $4, $5];

        /* replace fences by `null` in case of empty string: */
        if (parsed_content[0].length === 0) {
          parsed_content[0] = null;
        }
        if (parsed_content[2].length === 0) {
          parsed_content[2] = null;
        }
        id = _this.XXX_raw_id_from_content('command', raw_content, parsed_content);
        return $1 + "\x12" + id + "\x13";
      };
    })(this));
    return R;
  ...
  this.XXX_raw_id_from_content = function(collection_name, raw_content, parsed_content) {
    var R, fragment_by_ids, id_by_fragments;
    if (parsed_content == null) {
      parsed_content = null;
    }
    switch (collection_name) {
      case 'comment':
        fragment_by_ids = this.XXX_comment_by_ids;
        id_by_fragments = this.XXX_id_by_comments;
        break;
      case 'raw':
        fragment_by_ids = this.XXX_raw_content_by_ids;
        id_by_fragments = this.XXX_raw_id_by_contents;
        break;
      case 'command':
        fragment_by_ids = this.XXX_command_by_ids;
        id_by_fragments = this.XXX_id_by_commands;
        break;
      default:
        throw new Error("unknown collection collection_name " + (rpr(collection_name)));
    }
    if ((R = id_by_fragments.get(raw_content)) == null) {
      R = fragment_by_ids.size;
      fragment_by_ids.set(R, parsed_content != null ? parsed_content : raw_content);
      id_by_fragments.set(raw_content, R);
    }
    return R;
  };

  this.XXX_register_content = function(kind, raw, parsed, meta) {
    var collection, entry, id;
    collection = this.XXX_registry.get(kind);

    /* should never happen */
    if (collection == null) {
      throw new Error("unknown MKTS.PRE kind " + (rpr(kind)));
    }
    id = collection.size;
    entry = {
      raw: raw,
  ...
  this.$XXX_expand_html_comments = function(text) {

    /* TAINT code duplication */
    return $((function(_this) {
      return function(event, send) {
        var comment, i, id, is_comment, len, meta, name, ref, results, stretch, type;
        if (_this.select(event, '.', ['text', 'code'])) {
          is_comment = true;
          type = event[0], name = event[1], text = event[2], meta = event[3];
          ref = text.split(_this.XXX_html_comment_id_pattern);
          results = [];
          for (i = 0, len = ref.length; i < len; i++) {
            stretch = ref[i];
            ...
            if (is_comment) {
              ...
              comment = _this.XXX_comment_by_ids.get(id);

              /* should never happen: */
              if (comment == null) {
                throw new Error("unknown ID " + (rpr(stretch)));
              }
              results.push(send(['.', 'comment', comment, _this.copy(meta)]));
            } else {
              if (stretch.length !== 0) {
                results.push(send([type, name, stretch, _this.copy(meta)]));
              } else {
                results.push(void 0);
              }
            }
  ...
  this.$XXX_expand_commands = function(text) {
    return $((function(_this) {
      return function(event, send) {
        var command, command_name, fence, i, id, is_command, left_fence, len, meta, name, ref, results, right_fence, stretch, type;
        if (_this.select(event, '.', ['text', 'code', 'comment'])) {
          is_command = true;
          type = event[0], name = event[1], text = event[2], meta = event[3];
          ref = text.split(_this.XXX_command_id_pattern);
          results = [];
          for (i = 0, len = ref.length; i < len; i++) {
            stretch = ref[i];
            ...
            if (is_command) {
              id = parseInt(stretch, 10);
              command = _this.XXX_command_by_ids.get(id);

              /* should never happen: */
              if (command == null) {
                throw new Error("unknown ID " + (rpr(stretch)));
              }
              if (!CND.isa_list(command)) {
                throw new Error("not registered correctly: " + (rpr(stretch)));
              }
              left_fence = command[0], command_name = command[1], right_fence = command[2];
              fence = left_fence != null ? left_fence : right_fence;
              results.push(send([fence, command_name, null, _this.copy(meta)]));
            } else {
              if (stretch.length !== 0) {
                results.push(send([type, name, stretch, _this.copy(meta)]));
              } else {
  ...
  this.$XXX_expand_raw_spans = function(state) {
    return $((function(_this) {
      return function(event, send) {
        var content, i, id, is_raw, len, meta, name, ref, results, stretch, text, type;
        if (_this.select(event, '.', ['text', 'code', 'comment'])) {
          is_raw = true;
          type = event[0], name = event[1], text = event[2], meta = event[3];
          ref = text.split(_this.XXX_raw_id_pattern);
          results = [];
          for (i = 0, len = ref.length; i < len; i++) {
            stretch = ref[i];
            is_raw = !is_raw;
            if (is_raw) {
              id = parseInt(stretch, 10);
              content = _this.XXX_raw_content_by_ids.get(id);

              /* should never happen: */
              if (content == null) {
                throw new Error("unknown ID " + (rpr(stretch)));
              }
              results.push(send(['.', 'raw', content, _this.copy(meta)]));
            } else {
              results.push(send([type, name, stretch, _this.copy(meta)]));
            }
          }
  ...
  this.XXX_escape_escape_chrs = function(text) {
    var R;
    R = text;
  ...
  this.XXX_unescape_escape_chrs = function(text) {
    var R;
    R = text;
  ...
  this.create_mdreadstream = function(md_source, settings) {
    ...
    confluence.pipe(this.$_flatten_tokens(state)).pipe(this.$_reinject_html_blocks(state)).pipe(this.$_rewrite_markdownit_tokens(state)).pipe(this.$XXX_expand_html_comments(state)).pipe(this.$XXX_expand_commands(state)).pipe(this.$XXX_expand_raw_spans(state)).pipe(this.$_process_end_command(state)).pipe(R);
    R.on('resume', (function(_this) {
      return function() {
        ...
        environment = {};
        md_source = _this.XXX_escape_html_comments_raw_spans_and_commands(md_source);
        debug('comment_by_ids     ', _this.XXX_comment_by_ids);
        debug('raw_content_by_ids ', _this.XXX_raw_content_by_ids);
        debug('command_by_ids     ', _this.XXX_command_by_ids);
