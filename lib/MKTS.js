(function() {
  ...
  this.$XXX_expand_commands = function(text) {
    return $((function(_this) {
      return function(event, send) {
        var command, i, id, is_command, left_fence, len, meta, name, ref, results, right_fence, stretch, type;
        if (_this.select(event, '.', ['text', 'code', 'comment'])) {
          ...
          for (i = 0, len = ref.length; i < len; i++) {
            ...
            if (is_command) {
              ...
              left_fence = command[0], name = command[1], right_fence = command[2];
              results.push(send([left_fence, name, stretch, _this.copy(meta)]));
            } else {
  ...
  this.$XXX_unescape_raw_spans = function(state) {
    return $((function(_this) {
      return function(event, send) {
        var meta, name, text, type;
        if (_this.select(event, '.', ['text', 'code', 'comment', 'raw'])) {
          type = event[0], name = event[1], text = event[2], meta = event[3];
          event[2] = _this.XXX_unescape_raw_spans(text);
        }
        return send(event);
      };
    })(this));
  ...
  this.XXX_unescape_raw_spans = function(text) {
    var R;
    R = text;
    R = text.replace(this.XXX_raw_id_pattern, (function(_this) {
      return function(_, id_txt) {
        var id;
        id = parseInt(id_txt, 10);
        R = _this.XXX_raw_content_by_ids.get(id);
        if (R == null) {
          throw new Error("unknown ID " + (rpr(id_txt)));
        }
        return R;
      };
    })(this));
    R = this.XXX_unescape_escape_chrs(R);
    return R;
  };
  ...
  this.create_mdreadstream = function(md_source, settings) {
    ...
    confluence.pipe(this.$_flatten_tokens(state)).pipe(this.$_reinject_html_blocks(state)).pipe(this.$_rewrite_markdownit_tokens(state)).pipe(this.$XXX_expand_commands(state)).pipe(this.$XXX_unescape_raw_spans(state)).pipe(D.$show()).pipe(this.$_process_end_command(state)).pipe(R);
