(function() {
  ...
  this._tex_escape_replacements = [[/\\/g, '\\textbackslash{}'], [/\{/g, '\\{'], [/\}/g, '\\}'], [/\$/g, '\\$'], [/\#/g, '\\#'], [/%/g, '\\%'], [/_/g, '\\_'], [/\^/g, '\\textasciicircum{}'], [/~/g, '\\textasciitilde{}'], [/‰/g, '\\permille{}'], [/&amp;/g, '\\&'], [/&quot;/g, '"'], [/'([^\s]+)’/g, '‘$1’'], [/&/g, '\\&']];
  ...
  this.TRACKER.parse = (function(_this) {
    return function(pattern) {
      var _, left_fence, match, name, right_fence;
      left_fence = null;
      name = null;
      right_fence = null;
    ...
    return function() {
      ...
      (function() {
        ...
        for (i = 0, len = patterns.length; i < len; i++) {
          pattern = patterns[i];
          ref = _MKTS.TRACKER.parse(pattern), left_fence = ref[0], pattern_name = ref[1], right_fence = ref[2];
          state = {
  ...
  this.new_area_observer = function() {
    var area_key, area_name, area_names, closing_fence, fence_matcher, i, len, opening_fence, regular_closing_fence, state, track, within;
    ...
    for (i = 0, len = area_names.length; i < len; i++) {
      ...
      if ((regular_closing_fence = this.FENCES._get_opposite(opening_fence, null)) != null) {
        if (closing_fence !== regular_closing_fence) {
          throw new Error("unmatched fences in " + (rpr(area_name)));
        }
        if (area_names.length !== 1) {
          throw new Error("matching more than a single area name with fences not yet implemented");
        }
        area_key = area_name;
        area_name = area_name.slice(1, area_name.length - 1);
    ...
    if (opening_fence != null) {
      track = (function(_this) {
        return function(event) {
          var meta, text, type;
          if (event != null) {
            if (_this.isa(event, fence_matcher, area_name)) {
              type = event[0], area_name = event[1], text = event[2], meta = event[3];
              if (type === opening_fence) {
                state[area_key] = true;
              } else {
                state[area_key] = false;
              }
  ...
  this.$_preprocess_regions = function(S) {
    var closing_pattern, collector, opening_pattern, ref, region_stack, track, within;
    opening_pattern = /^@@@(\S.+)(\n|$)/;
    closing_pattern = /^@@@\s*(\n|$)/;
    collector = [];
    region_stack = [];
    ref = this.new_area_observer('(code)'), track = ref[0], within = ref[1];
    return $((function(_this) {
      return function(event, send) {
  ...
  this.$_preprocess_commands = function(S) {
    var collector, pattern, ref, track, within;
    pattern = /^∆∆∆(\S.+)(\n|$)/;
    collector = [];
    ref = this.new_area_observer('(code)'), track = ref[0], within = ref[1];
    return $((function(_this) {
  ...
  this.create_mdreadstream = function(md_source, settings) {
    ...
    confluence.pipe(this.$_flatten_tokens(state)).pipe(this.$_reinject_html_blocks(state)).pipe(this.$_rewrite_markdownit_tokens(state)).pipe(this.$_preprocess_commands(state)).pipe(this.$_process_end_command(state)).pipe(this.$_preprocess_regions(state)).pipe(this.$_close_dangling_open_tags(state)).pipe(R);
