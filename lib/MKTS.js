(function() {
    ...
    return $((function(_this) {
      return function(event, send) {
        ...
        if ((!within_code) && (_this.select(event, '.', 'text'))) {
          ...
          _this._flush_text_collector(send, collector, _this.copy(meta));
        } else if ((region_stack.length > 0) && (_this.select(event, '>', 'document'))) {
        ...
        if ((!within_code) && _this.select(event, '.', 'text')) {
          ...
          for (i = 0, len = lines.length; i < len; i++) {
            ...
            if ((match = line.match(pattern)) != null) {
              _this._flush_text_collector(send, collector, _this.copy(meta));
              send(['∆', match[1], null, _this.copy(meta)]);
            } else {
              collector.push(line);
            }
          }
        ...
        if (_this.select(event, ['{', '[', '('])) {
          ...
          if (_this.select(event, '>', 'document')) {
            while (tag_stack.length > 0) {
              ...
              send([sub_type, sub_name, null, _this.copy(sub_meta)]);
            }
            send(event);
          } else {
            tag_stack.pop();
  ...
  this.copy = function() {
    ...
    if ((isa_list = CND.isa_list(x))) {
      ...
      throw new Error("unable to cpy a " + (CND.type_of(x)));
    }
