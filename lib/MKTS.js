(function() {
  ...
  MKTS.XXX_raw_bracketed_pattern = /(?:(^|[^\\])<<\(raw>><<raw\)>>)|(?:(^|[^\\])<<\(raw>>(.*?[^\\])<<raw\)>>)/g;

  MKTS.XXX_raw_heredoc_pattern = /(^|[^\\])<<!raw:([^\s>]*)>>(.*?)\2/g;

  MKTS.XXX_raw_id_pattern = /\x11([0-9]+)\x13/g;

  MKTS.XXX_command_id_pattern = /\x12([0-9]+)\x13/g;

  MKTS.XXX_command_pattern = /(^|[^\\])(<<([!{[(]?)([^\s!{[()\]>}]+?)([)\]}]?)>>)/g;

  MKTS.XXX_escape_raw_spans = function(text) {
    var R;
    R = text;
  ...
  MKTS.XXX_raw_id_from_content = function(collection_name, raw_content, parsed_content) {
  ...
  MKTS.XXX_expand_commands = function(text) {
    var R, command, i, id, is_command, left_fence, len, name, ref, right_fence, stretch;
    is_command = true;
    R = [];
    ref = text.split(this.XXX_command_id_pattern);
    for (i = 0, len = ref.length; i < len; i++) {
      stretch = ref[i];
      is_command = !is_command;
      if (is_command) {
        id = parseInt(stretch, 10);
        command = this.XXX_command_by_ids.get(id);
  ...
  MKTS.XXX_unescape_raw_spans = function(text) {
    var R;
    R = text;
  ...
  MKTS.XXX_escape_escape_chrs = function(text) {
    var R;
    R = text;
  ...
  MKTS.XXX_unescape_escape_chrs = function(text) {
    var R;
    R = text;
  ...
  this.create_mdreadstream = function(md_source, settings) {
    ...
    confluence.pipe(this.$_flatten_tokens(state)).pipe(this.$_reinject_html_blocks(state)).pipe(this.$_rewrite_markdownit_tokens(state)).pipe(this.$_process_end_command(state)).pipe(R);
