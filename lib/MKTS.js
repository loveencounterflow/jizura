(function() {
  ...
  this.XXX_registry_index = new Map();
  ...
  this.XXX_do_bracketed_pattern = /(?:(^|[^\\])<<\{do>><<do\}>>)|(?:(^|[^\\])<<\{do>>([\s\S]*?[^\\])<<do\}>>)/g;


  /* Code duplication */

  this.XXX_raw_bracketed_pattern = /(?:(^|[^\\])<<\(raw>><<raw\)>>)|(?:(^|[^\\])<<\(raw>>([\s\S]*?[^\\])<<raw\)>>)/g;
  ...
  this.XXX_raw_id_pattern = /\x15raw([0-9]+)\x13/g;


  /* Code duplication */

  this.XXX_html_comment_id_pattern = /\x15comment([0-9]+)\x13/g;


  /* Code duplication */

  this.XXX_do_id_pattern = /\x15do([0-9]+)\x13/g;


  /* Code duplication */

  this.XXX_action_id_pattern = /\x15action([0-9]+)\x13/g;
  ...
  this.XXX_escape_html_comments_raw_spans_and_commands = function(text) {
    ...
    R = R.replace(this.XXX_html_comment_pattern, (function(_this) {
      return function(_, $1, $2, $3) {
        ...
        key = _this.XXX_register_content('comment', raw_content.trim());
    ...
    R = R.replace(this.XXX_do_bracketed_pattern, (function(_this) {
      return function(_, $1, $2, $3) {
        ...
        id = _this.XXX_register_content('do', raw_content);
        return $1 + "\x15" + id + "\x13";
      };
    })(this));
    R = R.replace(this.XXX_raw_bracketed_pattern, (function(_this) {
      return function(_, $1, $2, $3) {
        ...
        $1 += $2;
        raw_content = $3 != null ? $3 : '';
        id = _this.XXX_register_content('raw', raw_content);
        return $1 + "\x15" + id + "\x13";
      };
    })(this));
    ...
    R = R.replace(this.XXX_command_pattern, (function(_this) {
      return function(_, $1, $2, $3, $4, $5) {
        ...
        key = _this.XXX_register_content('action', raw_content, parsed_content);
        return $1 + "\x15" + key + "\x13";
      };
    })(this));
    return R;
  ...
  this.XXX_register_content = function(kind, raw, parsed) {
    ...
    id = this.XXX_registry_index.get(raw);
    if (id != null) {
      entry = this.XXX_registry[id];
      key = entry.key;
    } else {
      ...
      this.XXX_registry_index.set(raw, id);
    }
    return key;
  };

  this.XXX_retrieve_entry = function(id) {
    var R;
    if ((R = this.XXX_registry[id]) == null) {
  ...
  this.$XXX_expand_html_comments = function(text) {
    ...
    return $((function(_this) {
      return function(event, send) {
        var content, entry, i, id, is_comment, len, meta, name, ref, results, stretch, type;
        if (_this.select(event, '.', ['text', 'code'])) {
          ...
          for (i = 0, len = ref.length; i < len; i++) {
            ...
            if (is_comment) {
              ...
              entry = _this.XXX_retrieve_entry(id);
              content = entry['raw'];
  ...
  this.$XXX_expand_actions = function(text) {

    /* TAINT code duplication */
    return $((function(_this) {
      return function(event, send) {
        var action_name, content, entry, fence, i, id, is_command, left_fence, len, meta, name, ref, results, right_fence, stretch, type;
        if (_this.select(event, '.', ['text', 'code', 'comment'])) {
          is_command = true;
          type = event[0], name = event[1], text = event[2], meta = event[3];
          ref = text.split(_this.XXX_action_id_pattern);
          results = [];
          for (i = 0, len = ref.length; i < len; i++) {
            stretch = ref[i];
  ...
  this.$XXX_expand_do_spans = function(state) {
    ...
    return $((function(_this) {
      return function(event, send) {
        ...
        if (_this.select(event, '.', ['text', 'code', 'comment'])) {
          is_do = true;
          type = event[0], name = event[1], text = event[2], meta = event[3];
          ref = text.split(_this.XXX_do_id_pattern);
          results = [];
          for (i = 0, len = ref.length; i < len; i++) {
            stretch = ref[i];
            is_do = !is_do;
  ...
  this.create_mdreadstream = function(md_source, settings) {
    ...
    confluence.pipe(this.$_flatten_tokens(state)).pipe(this.$_reinject_html_blocks(state)).pipe(this.$_rewrite_markdownit_tokens(state)).pipe(this.$XXX_expand_html_comments(state)).pipe(this.$XXX_expand_actions(state)).pipe(this.$XXX_expand_raw_spans(state)).pipe(this.$XXX_expand_do_spans(state)).pipe(this.$_process_end_command(state)).pipe(R);
    R.on('resume', (function(_this) {
      return function() {
        ...
        urge('registry           ', _this.XXX_registry);
        urge('registry_index     ', _this.XXX_registry_index);
